...
@section('content')
<form action="{{route('credentials.update')}}" method="post" enctype="multipart/form-data" class="validation" novalidate>
    <div class="row mb-3">@csrf
      <input type="hidden" name="id" id="id" value="{{$credential->id}}" />
        <div class="col">
            <label for="">Title</label>
            <input type="text" name="title" class="form-control" placeholder="Title" value="{{$credential->title}}" required>
        </div>
        <div class="col">
            <label for="">Category</label>
            <select name="category" id="category" class="form-control" required>
                @foreach ($category as $cate)
                    <option value="{{$cate->id}}" @if($credential->category_id == $cate->id) selected @endif >{{$cate->name}}</option>
                @endforeach
